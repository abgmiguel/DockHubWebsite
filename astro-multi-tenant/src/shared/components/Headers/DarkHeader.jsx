...
const DarkHeader = ({ data = {} }) => {
  const { 
    logo = {},
    tagline = '',
    social = [],
    buttons = [],
    blogHomeButton = { href: '/blog', text: 'Blog Home' }
  } = data;

  // Check if we're in a blog post
  ...
  return (
    <nav className="sticky top-0 w-full bg-[#111111]/95 backdrop-blur supports-[backdrop-filter]:bg-[#111111]/60 z-50">
      <div className="relative flex h-12 items-center px-4">
        {/* Left - Conditional Logo/Home Button */}
        {isInBlogPost ? (
          <a 
            href={blogHomeButton.href}
            className="flex items-center gap-2 text-gray-200 hover:text-white transition-colors"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
            <span className="font-medium">{blogHomeButton.text}</span>
          </a>
        ) : (
          logo.src ? (
        ...
        <div className="absolute top-2 right-4 flex items-center gap-2">
          ...
          {buttons.map((button) => (
            <a
              key={button.text}
              href={button.href}
              target={button.external !== false ? "_blank" : undefined}
              rel={button.external !== false ? "noopener noreferrer" : undefined}
